...
public class Semantics { 
    ...
    State M (Program p) {
        return M (p.body/*, initialState(p.body.members)*/);  
    } 
   /*
    State initialState (ArrayList<Statement> membersBlock ) { 
        State state = new State(); 
        Value intUndef = new IntValue();
        for (Statement sta: membersBlock){
        	if(membersBlock.toString()=="Assignment"){
        		state.put(key, value);
        	}
        }
